...
export async function POST(request: Request) {
  ...
  try {
    ...
    const ORDER_SHIPPING_ADDRESS =
      requestBody.formData.street_address +
      ',' +
      requestBody.formData.city +
      ',' +
      requestBody.formData.state +
      ',' +
      requestBody.formData.zip_code;
    ...
    console.error(e);
    return NextResponse.json({
      error: 'Server side error occured when creating order.',
    });
  }
}

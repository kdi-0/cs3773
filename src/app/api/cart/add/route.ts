    ...
    if (product.PRODUCT_QUANTITY > 0) {
      ...
      if (cartItems != null) {
        ...
        existingItemIndex = cartItems.findIndex(
          (item) => item.PRODUCT_ID === requestBody.PRODUCT_ID
        );
      } else {
        //    item.PRODUCT_ID producing null error when trying to be accessed in case of cart being empty (FIXED this problem)
      ...
      if (existingItemIndex !== -1) {
        ...
        if (
          cartItems[existingItemIndex].PRODUCT_QUANTITY +
            productInfo.PRODUCT_QUANTITY <=
          product.PRODUCT_QUANTITY
        ) {
          cartItems[existingItemIndex].PRODUCT_QUANTITY = parseInt(
            cartItems[existingItemIndex].PRODUCT_QUANTITY
          );
          cartItems[existingItemIndex].PRODUCT_QUANTITY +=
            productInfo.PRODUCT_QUANTITY;
          cartItems[existingItemIndex].PRODUCT_PRICE =

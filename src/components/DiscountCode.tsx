...
const DiscountCode = ({
  ...
  const handleDiscountClick = async () => {
    ...
    if (discountCode.length != 0) {
      //there is a discount code trying to be applied
      ...
      try {
        ...
        if (
          discountResponse.data.discountExists == true &&
          !appliedDiscountCodes.includes(discountResponse.data.discountCode)
        ) {
          let discountResPercent =
            discountPercent + discountResponse.data.discountValue;
          let discountAmount =
            before_Tax_Order_total_price * (discountResPercent / 100);
          ...
          let price_with_discount =
            before_Tax_Order_total_price - discountAmount;
  ...
  return (
    <div>
      <input
        ...
        maxLength={20}
      />
      <button className="border font-semibold" onClick={handleDiscountClick}>
        Apply Discount
      </button>
      <div></div>
    </div>
